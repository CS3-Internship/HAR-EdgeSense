...
> **Prerequisite if each edge server has its own Wi-Fi hotspot (different SSID)**: none of the above triggers unless Android actually switches Wi-Fi networks as the phone moves — and by default, Android does *not* roam between two different SSIDs on its own. See [`frontend/README.md#edge-server-networks-wi-fi-roaming-across-hotspots`](frontend/README.md#edge-server-networks-wi-fi-roaming-across-hotspots): register each hotspot's SSID/password in the app (**System Information → Edge Server Networks**) so Android is told to treat them as one roamable set.
