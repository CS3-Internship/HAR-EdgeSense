...
EdgeSense supports deploying the backend on **multiple edge servers**, each its own Docker container (e.g. one per hotspot/location). Each keeps its own `./data` volume — the per-session JSON history and in-RAM buffer for a session live wherever that session started. When a phone roams onto a different edge server's network, that session would normally break: the new container's `./data` is empty and its buffer starts cold.

To keep the session alive, the Flutter app runs a small **fuzzy-logic handover controller** that fuzzifies Wi-Fi signal strength (RSSI) and request latency into an urgency score, and uses it to migrate the session to the new edge server the moment a network change is detected — pre-fetching everything ahead of time once the connection looks like it's degrading, so there's minimal gap in inference. Two things move on handoff: the small in-RAM state (buffer/step count/last prediction) via a JSON snapshot API, and the session's **history file** (`./data/sessions/session_{id}.json` — a plain JSON array, easy to inspect or hand-edit) — downloaded whole from the old container and uploaded whole into the new container's `./data` volume, so history migrates byte-for-byte rather than being replayed row by row. See [`backend/README.md#multi-edge-server-handover`](backend/README.md#multi-edge-server-handover) for the full design and the four `/session/{id}/...` endpoints, and [`frontend/lib/services/handover_controller.dart`](frontend/lib/services/handover_controller.dart) / [`fuzzy_handover.dart`](frontend/lib/services/fuzzy_handover.dart) for the client-side logic.

> **Prerequisite if each edge server has its own Wi-Fi hotspot (different SSID)**: register each hotspot in the app (**System Information → Edge Server Networks**) with its Wi-Fi name, password, and **explicit edge server address**. This isn't optional cosmetic setup — without it, Android won't switch Wi-Fi networks on its own (it only roams within a set of networks it's told are interchangeable), and even a manual switch may not trigger migration, since phone hotspots commonly share the same default gateway IP, which would otherwise make two different edge servers resolve to the same guessed address. See [`frontend/README.md#edge-server-networks-wi-fi-switching-across-hotspots`](frontend/README.md#edge-server-networks-wi-fi-switching-across-hotspots) for how the app actively drives the Wi-Fi switch itself rather than waiting on Android.
